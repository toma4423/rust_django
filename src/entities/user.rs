...
use super::{permission, user_permission, group_permission, group_user, group};
...
impl Model {
    ...
    pub async fn has_perm(&self, db: &DatabaseConnection, perm_codename: &str) -> Result<bool, DbErr> {
        ...
        // 2. 権限チェック (直接付与 + グループ経由)
        // ⚡ 最適化: 複数のクエリを1つのクエリに統合し、DB往復回数を削減します。
        // SELECT count(*) FROM permissions
        // LEFT JOIN user_permissions ON permissions.id = user_permissions.permission_id
        // LEFT JOIN group_permissions ON permissions.id = group_permissions.permission_id
        // LEFT JOIN group_user ON group_permissions.group_id = group_user.group_id
        // WHERE permissions.codename = $1 AND (user_permissions.user_id = $2 OR group_user.user_id = $2)
        let has_perm = permission::Entity::find()
            .join(JoinType::LeftJoin, permission::Relation::UserPermission.def())
            .join(JoinType::LeftJoin, permission::Relation::GroupPermission.def())
            .join(JoinType::LeftJoin, group_permission::Relation::Group.def())
            .join(JoinType::LeftJoin, group::Relation::GroupUsers.def())
            .filter(permission::Column::Codename.eq(perm_codename))
            .filter(
                Condition::any()
                    .add(user_permission::Column::UserId.eq(self.id))
                    .add(group_user::Column::UserId.eq(self.id))
            )
            .count(db)
            .await?;
